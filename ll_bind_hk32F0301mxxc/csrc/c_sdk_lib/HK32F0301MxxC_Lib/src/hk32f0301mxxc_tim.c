...
void TIM_DeInit(TIM_TypeDef* TIMx)
{
  ...
  if (TIMx == TIM1)
  {
    RCC_APBPeriph2ResetCmd(RCC_APBPeriph2_TIM1, ENABLE);
    RCC_APBPeriph2ResetCmd(RCC_APBPeriph2_TIM1, DISABLE);  
  }     
  else if (TIMx == TIM2)
  {
    RCC_APBPeriph1ResetCmd(RCC_APBPeriph1_TIM2, ENABLE);
    RCC_APBPeriph1ResetCmd(RCC_APBPeriph1_TIM2, DISABLE);
  }
  else if (TIMx == TIM6)
  {
    RCC_APBPeriph1ResetCmd(RCC_APBPeriph1_TIM6, ENABLE);
    RCC_APBPeriph1ResetCmd(RCC_APBPeriph1_TIM6, DISABLE);
  }  
...
void TIM_OC2Init(TIM_TypeDef* TIMx, TIM_OCInitTypeDef* TIM_OCInitStruct)
{
  ...
  /* Get the TIMx CCMR1_Output register value */
  tmpccmrx = TIMx->CCMR1_Output;
...
void TIM_OC3Init(TIM_TypeDef* TIMx, TIM_OCInitTypeDef* TIM_OCInitStruct)
{
  ...
  /* Get the TIMx CCMR2_Output register value */
...
void TIM_OC1PreloadConfig(TIM_TypeDef* TIMx, uint16_t TIM_OCPreload)
{
  ...
  /* Write to TIMx CCMR1_Output register */
  TIMx->CCMR1_Output = tmpccmr1;
}
...
void TIM_OC3PreloadConfig(TIM_TypeDef* TIMx, uint16_t TIM_OCPreload)
{
  ...
  /* Write to TIMx CCMR2_Output register */
  TIMx->CCMR2_Output = tmpccmr2;
}
                                ...
                                uint16_t TIM_IC1Polarity, uint16_t TIM_IC2Polarity)
{
  ...
  tmpccmr1 &= (uint16_t)(((uint16_t)~((uint16_t)TIM_CCMR1_Input_CC1S)) & (uint16_t)(~((uint16_t)TIM_CCMR1_Input_CC2S)));
  tmpccmr1 |= TIM_CCMR1_Input_CC1S_0 | TIM_CCMR1_Input_CC2S_0;
                       ...
                       uint16_t TIM_ICFilter)
{
  ...
  /* Write to TIMx CCMR1_Input and CCER registers */
  TIMx->CCMR1_Input = tmpccmr1;
  TIMx->CCER = tmpccer;
}
                       ...
                       uint16_t TIM_ICFilter)
{
  ...
  /* Write to TIMx CCMR2_Input and CCER registers */
                       ...
                       uint16_t TIM_ICFilter)
{
  ...
  TIMx->CCER = tmpccer;
	...
	/**
  * @brief  Specifies the TIMx Trigger source to ADC.
  * @param  TIMx: where x can be 1 to select the TIM peripheral.
  * @note     
  * @param  TIM_CC_TRIGADC_Channel: Configurate the CC_Channel to Trigger ADC.
	*          This parameter can be one of the following values:
  *            @arg TIM_Channel_1: Configurate the Channel_1
  *            @arg TIM_Channel_2: Configurate the Channel_2
  *            @arg TIM_Channel_3: Configurate the Channel_3
  *            @arg TIM_Channel_4: Configurate the Channel_4
	* @param  CC_Source: Specifies the TIMx Trigger source to ADC 
	*          This parameter can be one of the following values:
  *            @arg CC_TRIGADC_OC：indicate the Trigger source is OCx
  *            @arg CC_TRIGADC_OCREF：indicate the Trigger source is OCxREF
  * @retval None
  */
void TIM_CC_TRIGADC(TIM_TypeDef* TIMx, uint16_t TIM_CC_TRIGADC_Channel, uint32_t CC_Source)
{
  uint32_t tmpcr1 = 0;
  uint16_t channel;
  /* Check the parameters */
  assert_param(IS_TIM_LIST1_PERIPH(TIMx));
  assert_param(IS_TIM_CHANNEL(TIM_CC_TRIGADC_Channel));
  assert_param(IS_TIM_CC_TRIGADC_SOURCE(CC_Source));

	channel = TIM_CC_TRIGADC_Channel>>2;
	tmpcr1 = TIM1->CR1;
	/* Reset the CC_ADC_SEL Bits */
	tmpcr1 &= (~(CC_TRIGADC_MASK<<channel));
	/* Set the Counter Mode */
	tmpcr1 |= CC_Source<<channel;
	/* Write to TIMx CR1 register */
	TIMx->CR1 = tmpcr1;
}
