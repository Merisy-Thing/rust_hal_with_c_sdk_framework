...
use embedded_c_sdk_bind_hal::gpio::{FastPin, FastPinModeInput, FastPinModeOutput, FastPinReg, OutputFastPin};
use embedded_c_sdk_bind_hal::gpio::fast_pin_num::*;
use embedded_c_sdk_bind_hal::soft_i2c::{SoftI2C, FREQ_I2C_SCL_100K};
...
fn main() -> ! {
	...
	println!("bmp180 start");

	let mut bmp180;
	// let sda = Pin::new(PortNum::PB, PinNum::Pin14);
    // let scl = Pin::new(PortNum::PB, PinNum::Pin13);
	let sda: FastPin<PB, FastPin14, ()> = FastPin::new();
    let scl = FastPin::<PB, FastPin13, OutputFastPin>::new().into_output(FastPinModeOutput::OutOD);
    let soft_i2c = SoftI2C::<_, _, FREQ_I2C_SCL_100K>::new(scl, sda);
    let uninit_bmp180 = UninitBMP180::builder(soft_i2c, Delay::new()).mode(UltraHighResolution).build();
    ...
    loop {
        ...
        if key.is_low().unwrap_or(false) {
            delay.delay_ms(1000);
            println!("curr_tick = {}", Tick::tick() as u32);
        }
    }
}
