...
pub struct Reader<'a, T>(&'a RingBuffer<T>);

/// A type which can only write to a ring buffer.
pub struct Writer<'a, T>(&'a RingBuffer<T>);

impl<T> RingBuffer<T> {
    /// Create a new empty ringbuffer.
    pub const fn new() -> Self {
        Self {
            buf: AtomicPtr::new(core::ptr::null_mut::<T>()),
    ...
    pub unsafe fn reader(&self) -> Reader<'_, T> {
        Reader(self)
    }
    ...
    ///
    /// Only one reader can exist at a time.
    pub unsafe fn try_reader(&self) -> Option<Reader<'_, T>> {
    ...
    ///
    /// Only one writer can exist at a time.
    pub unsafe fn try_writer(&self) -> Option<Writer<'_, T>> {
        if self.buf.load(Ordering::Relaxed).is_null() {
            return None;
        }
        Some(Writer(self))
...
impl<'a, T> Writer<'a, T> {
    ...
    pub fn push(&mut self, f: impl FnOnce(&mut [T]) -> usize) -> usize {
        let (p, n) = self.push_buf();
        let buf = unsafe { slice::from_raw_parts_mut::<T>(p, n) };
        let n = f(buf);
        self.push_done(n);
        n
    }
    ...
    pub fn push_one(&mut self, val: T) -> bool {
    ...
    pub fn push_slice(&mut self) -> &mut [T] {
    ...
    pub fn push_slices(&mut self) -> [&mut [T]; 2] {
    ...
    pub fn push_buf(&mut self) -> (*mut T, usize) {
    ...
    /// The buffer stays valid as long as no other `Writer` method is called
    /// and `init`/`deinit` aren't called on the ringbuf.
    pub fn push_bufs(&mut self) -> [(*mut T, usize); 2] {
...
impl<'a, T: Copy> Reader<'a, T> {
    ...
    pub fn pop(&mut self, f: impl FnOnce(&[T]) -> usize) -> usize {
    ...
    pub fn pop_slice(&mut self) -> &mut [T] {
    ...
    /// and `init`/`deinit` aren't called on the ringbuf.
    pub fn pop_buf(&mut self) -> (*mut T, usize) {
